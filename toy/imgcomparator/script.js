...
class ImageComparator {
  ...
  compare () {
    const firstImgData = this.firstCanvasCtx.getImageData(-this.firstDX, -this.firstDY, this.width, this.height)
    const secondImgData = this.secondCanvasCtx.getImageData(-this.secondDX, -this.secondDY, this.width, this.height)
      ...
      switch (this.mode) {
        case 'strict': {
          if (
            ...
            diffImgData.data[i] = diffImgData.data[i + 1] = diffImgData.data[i + 2] = 0
          } else {
            diffImgData.data[i] = diffImgData.data[i + 1] = diffImgData.data[i + 2] = 255
            diffSum += 255 / l
          }
        ...
        case 'gray': {
          const diffAverage = fixRange(
            (Math.abs(firstR - secondR) + Math.abs(firstG - secondG) + Math.abs(firstB - secondB)) / 3
          )
          diffImgData.data[i] = diffImgData.data[i + 1] = diffImgData.data[i + 2] = diffAverage
          diffImgData.data[i + 3] = 255
  ...
  async loadImgAsync (firstSrc, secondSrc) {
    const drawImgToCanvasAsync = async (src, canvas, canvasCtx) => new Promise(resolve => {
...
;(async () => {
  ...
  await imageComparator.loadImgAsync('./examples/first-img.png', './examples/second-img.png')
  imageComparator.refresh()
})()

document.querySelectorAll('#displace-console .first button, #displace-console .second button').forEach(el => {
  el.addEventListener('click', function () {
    const displacement = this.dataset.displace.split(',').map(str => Number(str))
...
document.querySelector('#compare-img').addEventListener('click', async () => {
  if (firstImgFileInputer.files[0] && secondImgFileInputer.files[0]) {
    ...
    const secondDataUrl = await fileToDataUrlAsync(secondImgFileInputer.files[0])
    await imageComparator.loadImgAsync(firstDataUrl, secondDataUrl)
    imageComparator.refresh()
  }
})

document.querySelector('#displace-console .all .reset').addEventListener('click', () => {
  imageComparator.displaceTo(0, 0, 0, 0)
  imageComparator.refresh()
})

document.querySelector('#displace-console .all .auto-align').addEventListener('click', () => {

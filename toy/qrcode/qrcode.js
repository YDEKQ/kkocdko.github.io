...
function QRCode (
  ...
  QRCodeModel.createData = (typeNumber, errorCorrectLevel, dataList) => {
    ...
    if (buffer.getLengthInBits() > 8 * totalDataCount) {
      throw Error('code length overflow. (' + buffer.getLengthInBits() + '>' + 8 * totalDataCount + ')')
    }
  ...
  function _getTypeNumber (content, errorCorrectLevel) {
    ...
    for (let i = 0, len = QRCodeLimitLength.length; i <= len; i++) {
      ...
      if (!table) {
        throw Error('Content too long: expected ' + limit + ' but got ' + length)
      }
      switch (errorCorrectLevel) {
        case QRErrorCorrectLevel.L:
          limit = table[0]
  ...
  this.toSvg = function (moduleLength) {
    ...
    for (let y = 0; y < length; y++) {
      for (let x = 0; x < length; x++) {
        if (modules[x][y]) {
          svgContent += `<use href="#c" x="${x * moduleLength}" y="${y * moduleLength}"/>`
        }
      }
    }
    svgContent += '</svg>'
